...
//   Quill Editor Option
  ...
  const stepsConfig = [
      ...
      content: (
        <>
          <Form.Item name="title" label="Article Title" rules={[{ required: true, message: 'Please enter a title' }]}>
            <Input size="large" placeholder="e.g. Setting up the Development Environment" />
          </Form.Item>
          <Form.Item name="specialization" label="Category" rules={[{ required: true, message: 'Please enter specialization' }]}>
            <Input size="large" placeholder="e.g. React Development" prefix={<TagOutlinedIcon />} />
          </Form.Item>
          <Form.Item name="description" label="Short Summary" rules={[{ required: true, message: 'Please enter a description' }]}>
            <Input.TextArea rows={3} placeholder="A brief overview..." showCount maxLength={200} />
          </Form.Item>
        </>
      ),
    },
    {
      ...
      content: (
        <>
          <div style={{ marginBottom: 16, padding: '12px 16px', background: '#f9f9f9', border: '1px dashed #d9d9d9', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <Text strong>Import Content</Text>
              <div style={{ fontSize: 12, color: '#888' }}>Upload a .docx file to auto-fill</div>
            </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #001529 0%, #003a8c 100%)', padding: '40px 50px 80px', color: 'white', borderRadius: 12 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <Text style={{ color: 'rgba(255,255,255,0.65)' }}>Manage documentation, guides, and internal resources.</Text>
          </div>
          {isPrivileged && (
...
/>
      ...
      >
        <div style={{ padding: '24px 32px', borderBottom: '1px solid #f0f0f0', background: '#fafafa', borderRadius: '8px 8px 0 0' }}>
          ...
          <Text type="secondary">Share your knowledge with the team</Text>
        </div>
        
        <div style={{ padding: '24px 32px' }}>
          <Steps current={currentStep} size="small" style={{ marginBottom: 32 }} items={stepsConfig.map(s => ({ title: s.title }))} />
      ...
      >
        {viewingItem && (
            <>
                <div style={{ padding: '32px 40px 24px', background: '#fafafa', borderBottom: '1px solid #f0f0f0', borderRadius: '8px 8px 0 0' }}>
                ...
                <div style={{ padding: '16px 40px', borderTop: '1px solid #f0f0f0', display: 'flex', justifyContent: 'flex-end', background: '#fff', borderRadius: '0 0 8px 8px' }}>
                    <Button size="large" onClick={closeViewModal}>Close</Button>
                </div>
            </>
        )}
      </Modal>
